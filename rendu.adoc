...
Le second levier ne part pas d'un vide instrumental. Le dépôt montre au
contraire que Sentry est déjà branché à plusieurs niveaux du système. Côté
front, les erreurs API sont capturées avec des informations de contexte comme
l'identifiant de requête, l'URL appelée, le code de statut et, lorsqu'il est
disponible, l'utilisateur concerné. Côté backend, une bibliothèque transverse
alimente Sentry dans de nombreux microservices et y rattache d'emblée un tag
de microservice. Les interceptors en place enrichissent aussi les événements
selon le type de flux traité : requête HTTP, appel gRPC, message RabbitMQ ou
événement WebSocket. Cette base n'est donc ni improvisée ni marginale ; elle
témoigne d'une volonté réelle d'inscrire l'observabilité dans l'architecture
courante.
...
De même, le sujet CloudWatch est posé en termes d'amélioration des logs, de
couverture plus large des applications et de meilleure maîtrise du niveau de
verbosité. L'état observé correspond donc à une observabilité présente, mais
...
les rendre mutuellement fécondes : Sentry pour pointer rapidement le lieu
...
Il convient néanmoins de garder une lecture prudente du chantier. Les tickets
ouverts montrent que plusieurs mesures importantes restent à concrétiser. Pour
CloudWatch, la centralisation des logs de toutes les applications n'est pas
